...
{
    dim_ = dim;
    send_msg_to_other_EA_ = new real_float[dim + 1];
...
int Comm::RecvData(Message &message)
{
    ...
    if(message.flag == 0)
    {
        real_float * msg_recv = new real_float[message.msg_length];
...
int Comm::SendData(Message &message)
{
    if(message.flag == 0)
    {
        if(CheckSend() == 1)
        {
            ...
            send_msg_to_other_EA_ = new real_float[msg_length];
...
int Comm::DeserialMsgToIndividual(Population &individual, real_float *msg, int length)
{
    int count = 0;
    individual.clear();
...
int Comm::SerialIndividualToMsg(real_float *msg, Population &individual)
{
    int count = 0;
