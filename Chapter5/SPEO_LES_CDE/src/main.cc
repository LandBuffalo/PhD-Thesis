...
#ifdef SEQUENTIAL
    node_info.task_ID = 0;
    if(node_info.node_ID >= total_runs.size() * total_functions.size())
        node_info.task_ID = -1;
#endif
//---------------------------island info----------------------------------------------------//
#ifdef SEQUENTIAL
    island_info.island_num = 1;
#else
    #ifdef GPU_EA
    ...
    island_info.regroup_option.assign("regroup");
    ...
    island_info.buffer_capacity = island_info.buffer_capacity * island_info.island_size;
    island_info.model_num = island_info.island_size;
    if (getParam("-model_num", argv, argc))
        island_info.model_num = atof(getParam("-model_num", argv, argc));
    if (getParam("-k_nearest", argv, argc))
        island_info.k_nearest = atof(getParam("-k_nearest", argv, argc));
//---------------------------EA info----------------------------------------------------//
    EA_info.CR = 0.9;
    EA_info.F = 0.5;
...
#ifdef SEQUENTIAL
    problem_info.function_ID = node_info.node_ID / total_runs.size() + total_functions[0];
    problem_info.run_ID = node_info.node_ID % total_runs.size() + total_runs[0];
