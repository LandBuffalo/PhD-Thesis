...
vector<int> Migrate::BestOrWorst(Population &population, int select_num, string flag)
{
    ...
    {
        int index = 0;
        real_float fitness_value = tmp_population[0].fitness_value;
...
real_float Migrate::ElasticAsyncMigrate(Population &population, EA * EA, real_float percentage, long int generation)
{
    real_float communication_time = comm_.Time();
#ifdef SEQUENTIAL
    Population candidate;
    
#ifdef STANDARD_CDE
    EA->Reproduce(candidate, population);
    EA->EvaluateFitness(candidate);
    EA->SelectSurvival(population, candidate);
#else
    if(generation < 0.3 )
    {
        EA->Reproduce(candidate, population);
    ...
    {
        for (int i = 0; i < EA_info_.candidate_rate; i++)
        {
            EA->Reproduce(candidate, population);
        }
        buffer_manage_->MajorityVote(candidate, population);
    }
    EA->EvaluateFitness(candidate);
    EA->SelectSurvival(population, candidate);
    buffer_manage_->RecvData(candidate, population);
#endif    
...
int Migrate::RecvImmigrations(Population &population, EA * EA)
{
    ...
    while(comm_.CheckRecv(recv_msg, sender, tag) == 1)
    {
        ...
        buffer_manage_->RecvData(recv_msg.data, population);
    }
