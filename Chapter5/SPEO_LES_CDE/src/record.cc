...
#ifdef SEQUENTIAL
        file_name_ = "./Results/SPEO_sequential";
#else
    #ifdef GPU_EA
    #ifdef DUAL_CONTROL
...
int Record::PrintResult(vector< vector<Result> > result)
{
    ...
    for(int i = 0; i < merged_result.size(); i++)
        file<<problem_info_.function_ID<<','<<problem_info_.run_ID<<','<<problem_info_.dim<<','<<fixed<<setprecision(15)<<merged_result[i].FEVs<<','<<fixed<<setprecision(2)<<merged_result[i].computing_time<<','<<0.0<<','<<floor(merged_result[i].FEs)<<','<<node_info_.node_num<<','<<node_info_.GPU_num<<','<<island_info_.island_num<<','<<island_info_.island_size<<','<<island_info_.island_size<<','<<island_info_.interval<<','<<fixed<<setprecision(4)<<island_info_.migration_rate<<','<<fixed<<setprecision(4)<<island_info_.connection_rate<<','<<island_info_.buffer_capacity<<','<<island_info_.migration_topology<<','<<island_info_.buffer_manage<<','<<island_info_.regroup_option<<','<<EA_info_.EA_parameters<<endl;
#else
    for(int i = 0; i < merged_result.size(); i++)
...
int Record::RecordResult(real_float current_FEs, real_float computing_time, real_float comm_time, real_float FEVs)
{
    Result result;
    result.FEVs = FEVs;
...
int Record::RecordFlag(real_float current_FEs, real_float computing_time, vector<real_float> &record_criterion)
{
    int flag = 0;
...
int Record::GatherResults(int printer)
{
...
#ifdef SEQUENTIAL
    vector< vector<Result> > result(1, result_);
    ...
    if (node_info_.node_ID != printer)
    {
        ...
        real_float *msg = new real_float[4 * result_num];
    ...
    else
    {
        ...
        for(int i = 1; i < island_info_.island_num; i++)
        {
            ...
            real_float *msg = new real_float[length_msg];
