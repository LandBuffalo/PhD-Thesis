...
#ifndef GLOBAL_CEC2014
#define GLOBAL_CEC2014
double *OShift, *Mdata, *ye, *ze, *x_bound;
...
void cec14_test_func(double *pop, double *fval, int dim, int size_pop, int ID_func);

CEC2014::CEC2014(natural ID_func, natural size_pop, natural dim)
{
	ID_func_ = ID_func;
	size_pop_ = size_pop;
	maxbound_ = 100;
...
void CEC2014::set_pop_original_and_fval(double * pop_original, double * fval)
{
	pop_original_ = pop_original;
	fval_ = fval;
}
...
void CEC2014::EvaluateFitness(double * fval, double * pop)
{
	double *pop_transfer = new double[dim_];
	for (int i = 0; i < size_pop_; i++)
	{
		for (int j = 0; j < dim_; j++)
			pop_transfer[j] = pop[i + j*size_pop_];
		cec14_test_func(pop_transfer, fval + i, dim_, 1, ID_func_);
	}
	delete[] pop_transfer;
}
...
void cec14_test_func(double *x, double *f, int nx, int mx, int func_num)
{
	...
	for (i = 0; i < mx; i++)
	{
		...
		{
		case 1:
			ellips_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 1);
			f[i] += 100.0;
			break;
		case 2:
			bent_cigar_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 1);
			f[i] += 200.0;
			break;
		case 3:
			discus_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 1);
			f[i] += 300.0;
			break;
		case 4:
			rosenbrock_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 1, 2.048 / (double)100);
			f[i] += 400.0;
			break;
		case 5:
			ackley_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 1);
			f[i] += 500.0;
			break;
		case 6:
			weierstrass_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 0.5 / (double)100);
			f[i] += 600.0;
			break;
		case 7:
			griewank_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 6);
			f[i] += 700.0;
			break;
		case 8:
			rastrigin_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 0, 0, 5.12 / (double)100);
			f[i] += 800.0;
			break;
		case 9:
			rastrigin_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 5.12 / (double)100);
			f[i] += 900.0;
			break;
		case 10:
			schwefel_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 0, 0, 10);
			f[i] += 1000.0;
			break;
		case 11:
			schwefel_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 10);
			f[i] += 1100.0;
			break;
		case 12:
			katsuura_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 5 / (double)100);
			f[i] += 1200.0;
			break;
		case 13:
			happycat_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 5 / (double)100);
			f[i] += 1300.0;
			break;
		case 14:
			hgbat_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 0, 5 / (double)100);
			f[i] += 1400.0;
			break;
		case 15:
			grie_rosen_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 1, 5 / (double)100);
			f[i] += 1500.0;
			break;
		case 16:
			escaffer6_func(&x[i*nx], &f[i], nx, OShift, Mdata, 1, 1, 1, 1);
			f[i] += 1600.0;
			break;
		case 17:
			hf01(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1, 1);
			f[i] += 1700.0;
			break;
		case 18:
			hf02(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1, 1);
			f[i] += 1800.0;
			break;
		case 19:
			hf03(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1, 1);
			f[i] += 1900.0;
			break;
		case 20:
			hf04(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1, 1);
			f[i] += 2000.0;
			break;
		case 21:
			hf05(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1, 1);
			f[i] += 2100.0;
			break;
		case 22:
			hf06(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1, 1);
			f[i] += 2200.0;
			break;
		case 23:
			cf01(&x[i*nx], &f[i], nx, OShift, Mdata, 1);
			f[i] += 2300.0;
			break;
		case 24:
			cf02(&x[i*nx], &f[i], nx, OShift, Mdata, 1);
			f[i] += 2400.0;
			break;
		case 25:
			cf03(&x[i*nx], &f[i], nx, OShift, Mdata, 1);
			f[i] += 2500.0;
			break;
		case 26:
			cf04(&x[i*nx], &f[i], nx, OShift, Mdata, 1);
			f[i] += 2600.0;
			break;
		case 27:
			cf05(&x[i*nx], &f[i], nx, OShift, Mdata, 1);
			f[i] += 2700.0;
			break;
		case 28:
			cf06(&x[i*nx], &f[i], nx, OShift, Mdata, 1);
			f[i] += 2800.0;
			break;
		case 29:
			cf07(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1);
			f[i] += 2900.0;
			break;
		case 30:
			cf08(&x[i*nx], &f[i], nx, OShift, Mdata, SS, 1);
			f[i] += 3000.0;
			break;
		default:
...
void sphere_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Sphere */
{
	int i;
	f[0] = 0.0;
...
void ellips_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Ellipsoidal */
{
	int i;
	f[0] = 0.0;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
	...
	for (i = 0; i<nx; i++)
	{
		f[0] += pow(10.0, 6.0*i / (nx - 1))*ze[i] * ze[i];
	}
}

void bent_cigar_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Bent_Cigar */
{
	int i;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
...
void discus_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Discus */
{
	int i;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
...
void dif_powers_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Different Powers */
{
	int i;
	f[0] = 0.0;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
...
void rosenbrock_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Rosenbrock's */
{
	int i;
	double tmp1, tmp2;
	f[0] = 0.0;
...
void schaffer_F7_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Schwefel's 1.2  */
{
	int i;
	double tmp;
	f[0] = 0.0;
...
void ackley_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Ackley's  */
{
	int i;
	double sum1, sum2;
	sum1 = 0.0;
...
void weierstrass_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Weierstrass's  */
{
	int i, j, k_max;
	double sum, sum2, a, b;
	a = 0.5;
...
void griewank_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Griewank's  */
{
	int i;
	double s, p;
	s = 0.0;
...
void rastrigin_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Rastrigin's  */
{
	int i;
	f[0] = 0.0;
...
void step_rastrigin_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Noncontinuous Rastrigin's  */
{
	int i;
	f[0] = 0.0;
...
void schwefel_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Schwefel's  */
{
	int i;
	double tmp;
	f[0] = 0.0;
...
void katsuura_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Katsuura  */
{
	int i, j;
...
void bi_rastrigin_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Lunacek Bi_rastrigin Function */
{
	int i;
...
void grie_rosen_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Griewank-Rosenbrock  */
{
	int i;
...
void escaffer6_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Expanded Scaffer??s F6  */
{
	int i;
	double temp1, temp2;
...
void happycat_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* HappyCat, provdided by Hans-Georg Beyer (HGB) */
...
void hgbat_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* HGBat, provdided by Hans-Georg Beyer (HGB)*/
/* original global optimum: [-1,-1,...,-1] */
{
	int i;
	double alpha, r2, sum_z;
...
void hf01(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 1 */
{
	...
	i = 0;
	schwefel_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 1;
	rastrigin_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 2;
...
void hf02(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 2 */
{
	...
	i = 1;
	hgbat_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 2;
	rastrigin_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
...
void hf03(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 3 */
{
	...
	i = 0;
	griewank_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 1;
	weierstrass_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 2;
	rosenbrock_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 3;
...
void hf04(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 4 */
{
	...
	i = 2;
	grie_rosen_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 3;
	rastrigin_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
...
void hf06(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 6 */
{
	...
	i = 0;
	katsuura_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 1;
	happycat_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 2;
	grie_rosen_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
	i = 3;
	schwefel_func(&ye[G[i]], &fit[i], G_nx[i], Os, Mr, 0, 0, 0, 1);
...
void cf01(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 1 */
{
	...
	i = 0;
	rosenbrock_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, 1, 0, 1);
...
void cf02(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 2 */
{
	...
	i = 0;
	schwefel_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, 0, 0, 1);
	i = 1;
	rastrigin_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
	i = 2;
	hgbat_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
	cf_cal(x, f, nx, Os, delta, bias, fit, cf_num);
}
...
void cf03(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 3 */
{
	...
	double bias[3] = { 0, 100, 200 };
	i = 0;
	schwefel_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
...
void cf04(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 4 */
{
	...
	fit[i] = 1000 * fit[i] / 4e+3;
	i = 1;
	happycat_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
	fit[i] = 1000 * fit[i] / 1e+3;
	i = 2;
	ellips_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
	fit[i] = 1000 * fit[i] / 1e+10;
	i = 3;
	weierstrass_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
	fit[i] = 1000 * fit[i] / 400;
	i = 4;
	griewank_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
...
void cf06(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 6 */
{
	...
	double bias[5] = { 0, 100, 200, 300, 400 };
	i = 0;
	grie_rosen_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
	fit[i] = 10000 * fit[i] / 4e+3;
	i = 1;
	happycat_func(x, &fit[i], nx, &Os[i*nx], &Mr[i*nx*nx], 1, r_flag, 0, 1);
	fit[i] = 10000 * fit[i] / 1e+3;
